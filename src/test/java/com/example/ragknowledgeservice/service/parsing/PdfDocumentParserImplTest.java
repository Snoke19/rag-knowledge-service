...
public class PdfDocumentParserImplTest {
    ...
    void throwsDocumentParsingExceptionWhenPdfIsInvalid() {
        UUID documentId = UUID.randomUUID();
        when(documentService.downloadDocument(documentId)).thenReturn("not a pdf".getBytes());
    ...
    void wrapsPdfBoxFailureInDocumentParsingException() {
        ...
        when(documentService.downloadDocument(documentId))
            .thenReturn("invalid-pdf".getBytes(StandardCharsets.UTF_8));
